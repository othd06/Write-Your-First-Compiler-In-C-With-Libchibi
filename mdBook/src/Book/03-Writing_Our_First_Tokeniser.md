...
I recommend opening up the grammar for our tokeniser from last chapter as well as you follow along here. You can find it in the Grammar section of the references for this chapter, or as the bottom half of the grammar in the references for last chapter.

### Defining Our Tokens:

So from this we can start to define our tokens. You're first instinct might be to define a token using an enum. After all, an enum is the perfect data type for defining a set of discrete categories of thing. But remember, some of our tokens (such as literals) also need to store semantic meaning (and, down the line, perhaps other data such as their location for debugging and error messages). We will still use an enum but we that will use that to define the kind in order to form a [Tagged Union](https://en.wikipedia.org/wiki/Tagged_union). In your header section of tokeniser.h write the following:
...
As you can see our token contains two elements: the kind of token, and a union containing whatever semantic data the token may have. This is what will allow the parser to check which kind of token is being read in then correctly store the meaning of that token. Note that there is no whitespace token kind (or, for that matter, any token kind not used by the parser in the grammar from last chapter).

### Defining Our Helpers:

Now we need a function that can take in a string of chars and return a string of these tokens. For this we will use a data structure that I am calling a Seq but that you might know as a Vector from C++. We will be defining it in the header section of our helpers.h file:
```c
...
```
This may look a touch weird because we're using C preprocessor macros rather than functions or standard typedefs to define the Seq and the append operation. This is because we want to be able to declare Seq types for any underlying base type and we want our append to work for that type. Don't worry too much if you can't follow the exact details as it isn't the point of this book or chapter but I encourage you to attempt to do so anyway.
...
```
That's it. The entire signature. All of the complexity in our tokeniser lives inside the implementation. Finally, after all this setup, we can start to write our actual tokeniser. The tokeniser will run in a loop iterating through the string and emitting tokens as soon as they are ready. It is, after all, the role of the tokeniser to determine when a token is complete and ready to emit.

#### Setup:

We need a few things as setup though. Specifically our tokeniser will want an index to track how far through the string it is, a Seq of tokens to add new tokens to (and ultimately to return), and a dictionary of keywords to compare against. So let's build that.
```c
Token_Seq tokenise(char* plaintext) {
    int index = 0;
    Token_Seq tokens = {0};
...
```
For whitespace, since we don't need to actually emit our tokens on the iteration of the loop we are, effectively, cheating and using the main loop as the '*' in our grammar:
```EBNF
<whitespace> ::= (" " | "\t" | "\n" | "\r")*;
```
And for parens, since they are only ever one character it is easy to see how they already correspond directly with the grammar since we see that token, consume it with 'index++' and emit the token for that grammar symbol. This is the exact same pattern that we will see repeat everywhere our code corresponds to a pre-determined formal grammar.

#### Keywords And Identifiers

The other cases are a bit more complex. Lets look at if the character is alphabetical first. Clearly the first thing we have to do is build a string containing all the alphanumerical (or '_') characters that appear in an uninterrupted row. This, however, is not actually enough since our token grammar is technically ambiguous. In particular, there is no grammatical difference between a token, a keyword, and a boolean literal so we need to do some additional work to determine if we have a keyword or an identifier (work that is much easier to do here in the tokeniser than it would be later in the parser). This work is exactly why we defined our dictionary of keywords and created our helper function 'string_in_array'. To build the string itself we can use a Seq of chars and append to it each character until we find something that cannot be in our identifier, keyword, or boolean literal.
...
```
Notice how there are effectively two parts to this code. The first is a loop that consumes characters so long as they are alphabetical, digits, or '_'. This matches our grammar for identifiers closely:
``` EBNF
<identifier> ::= <letter> (<letter> | <digit> | "_")*;
```
With the loop matching the '*', the branch matching the first character, and the different ORed cases of the if branch representing the grouped alternatives.

The second half is then for null terminating and resolving the ambiguity by checking if our text matches some token other than an identifier and preferentially selecting that.

#### String Literals:

It is relatively trivial, now, to see how we would do the same thing for the string case since it doesn't even require the same disambiguating:
```c
//string_literal
else if (plaintext[index] == '"') {
...
```
We can even add the ability to escape quotes by checking if the last character in our string is a backslash like this:
```c
//string_literal
else if (plaintext[index] == '"') {
    index++; //consume the opening quote
...
```
Later we may want to return to this to allow us to escape other characters such as new lines, tabs, backslashes themselves, etc... (particularly in cases where the character we write for the escape is different than the one that get's embedded in the string).

#### Numeric Literals:

Now the only remaining case is when we find a leading digit.

If the token is an integer_literal it's simply a case of doing the same thing as before except instead of appending to a Seq of chars we multiply a running total by 10 then add the value of the digit (which we can get by taking the [ASCII](https://en.wikipedia.org/wiki/ASCII) encoding of the digit in our char, casting to a long, and subtracting [48](https://en.wikipedia.org/wiki/ASCII#Printable_character_table)). This is a feature of how the base 10 decimal system works where multiplying by ten is effectively shifting each digit to the left by 1.

We can then check if the next character is a '.'. If not, we emit the token as an integer literal. Otherwise, we have to do the same thing again to build a second integer then cast both integers to long floats, divide the second one by 10 until it is less than 1 (to shift it into the decimal part) and add them together. We divide by 10 until it is less than one to effectively shift our digits right until the largest is on the order of 0.1, or just to the right of the decimal point.
...
Note again how our loops represent the '+' in our grammar and we have to detect the '.' separately in order to resolve between the integer_literal and decimal_literal symbols since they start the same (hence their being in the same branch) even though they're not ambiguous.
...
[r_paren]
```
Note how our code directly corresponded with the grammar we had from last chapter. This shows how the grammar acts as a framework for writing our code as a direct consequence of the design we have already done in our specific formal notation.

Note as well how if we attempted to do what we did last chapter in devising and designing our syntax while directly writing our code without that grammatical notation we would very quickly get bogged down in a way that we don't after separating the process into separate design and implementation stages with grammar as the syntax that exists between them.

### Final Notes

Don't forget as well to import the correct headers at the start of the header section of tokeniser.h:
  * \<stdlib.h>
  * \<string.h>
  * \<stdio.h>
  * \<ctype.h>
...
Before we finish though, you will find that when writing the parser it's helpful to have a token denoting the end of the file. Adding this is very easy. Simply add an element TOK_EOF to the TokenKind enum and then, before returning from the tokenise function make sure to add an eof token:
```c
//return
Token eof_token;
