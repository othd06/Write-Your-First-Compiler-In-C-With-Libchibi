...
It would also be a pretty poor if our language couldn't work with local variables so we definitely need to be able to declare and assign to local variables. While we're at it let's also allow for local variable definitions although it's worth noting that in our Libchibi backend this will just become syntax sugar for a separate declaration and assignment.
